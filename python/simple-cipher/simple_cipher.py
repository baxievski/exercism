...
class Cipher(object):
    ...
    def encode(self, text):
        ...
        for i, character in enumerate(plain_text):
            ...
            encoded_character = chr(ord('a') + (ord(character) - ord('a') + shift_distance) % (ord('z') - ord('a') + 1))
    ...
    def decode(self, text):
        ...
        for i, character in enumerate(encoded_text):
            shift_distance = ord(self.key[i % len(self.key)]) - ord('a')
            decoded_character = chr(ord('a') + (ord(character) - ord('a') - shift_distance) % (ord('z') - ord('a') + 1))
